    ...
    operations: [
        new Post(
            ...
            openapi: new OpenApiOperation(
                ...
                description: "The body is the file itself, UTF-8, comma-, semicolon- or tab-separated, with a header row. Rows with the same `orderReference` (within a channel) are one order. Required columns: `orderReference`, `customerName`, `shippingLine1`, `shippingPostalCode`, `shippingCity`, `shippingCountry`, `sku`, `quantity`, `unitPrice` (major units, such as 199.00). Optional: `channel` (default `manual`), `placedAt`, `currency`, `location`, `customerEmail`, `shippingName`, `shippingLine2`, `shippingRegion`, `shippingPhone`, `lineName`. At most 5000 rows and 1 MiB.\n\nThe reference is stored as the order's `externalReference`, unique per channel: an order the channel already has is left alone, so importing the same file again creates nothing. Every order is checked like one created with `POST /api/orders` and written in its own transaction; an order with any problem is skipped whole and its rows are listed in `errors`. A problem with the file as a whole is a 422 and nothing is written.",
