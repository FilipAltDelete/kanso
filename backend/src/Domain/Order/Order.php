...
class Order
{
    ...
    public function cancelUnits(array $lines, ?string $reason, Actor $actor, \DateTimeImmutable $now): array
    {
        ...
        if (array_sum($cancelling) === $this->remainingUnits() && OrderStatus::OnHold === $this->status && $this->anyShipped()) {
        ...
        if (0 === $this->remainingUnits()) {
            $this->apply($this->anyShipped() ? Transition::Ship : Transition::Cancel, $actor, $now);
        }
    ...
    /** Whether any unit has shipped (counted on the lines, so a voided shipment would count back). */
    private function anyShipped(): bool
    {
        return array_sum(array_map(static fn (OrderLine $line): int => $line->shippedQuantity(), $this->lines())) > 0;
    }
