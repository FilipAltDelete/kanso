...
| OrderEvent | Audit trail: creation, every state change, shipments, notes, tag and payment status changes | order, type (`created`, `transition`, `shipment`, `note`, `tags_changed`, `payment_status_changed`), transition, actor + actor name, before/after, occurred_at |
