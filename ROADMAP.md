...
- [x] CSV order import: one row per line, grouped into orders by `orderReference`; upload, preview of every failing row, then import; each order through `OrderService` (same checks as a manual order, unknown SKUs refused) in its own transaction, skipped whole on any problem; `external_reference` on orders, unique per channel, so the same file twice creates nothing new; optional `paymentStatus`, `tags` (`|`-separated) and `note` columns set on the new order in its creating transaction, each with its order event; 5,000 rows / 1 MiB; `POST /api/order-imports` with `dryRun` (`docs/adr/0008`)
- [x] Editing orders, partial cancel: edit lines (quantities, add, remove), customer and addresses before picking starts and before anything ships, with the reservation following in the same transaction (`POST /api/orders/{id}/edits`); cancel units that have not shipped (`POST /api/orders/{id}/cancellations`), released from the reservation, kept on the line as `cancelledQuantity`; when nothing is left the order finishes as shipped or cancelled through the state machine; "Edit order" (`e`) and "Cancel items" dialogs on the order page (`docs/adr/0011`)
...
- [x] Pick lists and packing slips as PDFs (Swedish or English): Twig + dompdf in a worker, stored in S3/MinIO, downloaded through a five-minute signed link; "Print" on the order page; up to 100 orders in one PDF from the order list's bulk-action bar, leaving out and listing the ones that cannot be printed (`POST /api/orders/bulk-documents`) (`docs/adr/0007`)
